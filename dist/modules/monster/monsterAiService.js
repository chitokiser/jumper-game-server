...
function tickChasing(m, stepM) {
    // 타겟 유효성 재확인
    if (!(0, monsterTargetService_js_1.isTargetStillValid)(m)) {
        m.targetUserId = null;
        m.state = 'return';
        return m;
    }
    ...
    // 직선 이동
    const moved = (0, geo_js_1.moveToward)(m.currentLat, m.currentLng, target.lat, target.lng, stepM);
    m.currentLat = moved.lat;
    m.currentLng = moved.lng;
    return m;
}
function tickAttacking(m) {
    // 사거리 이탈 확인
    if (!(0, monsterTargetService_js_1.isTargetStillValid)(m)) {
        m.targetUserId = null;
        m.state = 'return';
...
function tickReturn(m, stepM) {
    const dist = (0, geo_js_1.haversineM)(m.currentLat, m.currentLng, m.spawnLat, m.spawnLng);
    ...
    // 스폰 위치로 이동
    const moved = (0, geo_js_1.moveToward)(m.currentLat, m.currentLng, m.spawnLat, m.spawnLng, stepM);
    m.currentLat = moved.lat;
    m.currentLng = moved.lng;
    // 복귀 중에도 어그로 체크
    const targetId = (0, monsterTargetService_js_1.findNearestTarget)(m);
